...
DEFAULT_SUMMARY = ROOT / "vldb_res" / "20260709000616(main)" / "summary.csv"
...
def is_baseline_row(row: dict[str, str]) -> bool:
    if row.get("workload", "") != "smallbank":
        return False
    if norm_float(row.get("access_pattern", "")) != "1":
        return False
    if not float_eq(row.get("account_count", ""), 10000000):
        return False
    if norm_float(row.get("worker_threads", "")) != "16":
        return False
    if norm_float(row.get("affinity_txn_ratio", "")) != "0.8":
        return False
    if norm_float(row.get("batch_size", "")) != "10000":
        return False
    if norm_float(row.get("key_page_ratio", "")) != "1.1":
        return False
...
def find_row(rows: list[dict[str, str]], theta: str, mode: str) -> dict[str, str] | None:
    matches = [
        ...
        and row.get("scan_axis", "") in {"base", "access"}
    ]
    if not matches:
        return None
    if len(matches) > 1:
        raise RuntimeError(
            f"Expected one latency row for theta={theta}, mode={mode}, "
            f"found {len(matches)}: {[(row.get('case_id'), row.get('scan_axis')) for row in matches]}"
        )
    return matches[0]
...
def make_latex(data) -> str:
    lines = [
        r"\begin{table}[t]",
        r"  \centering",
        r"  \caption{Latency percentiles (DB-side \& end-to-end).}",
        r"  \vspace{-5pt}",
